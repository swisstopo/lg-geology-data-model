...
input_file = "exports/2025_05_26/GCOVERP_export_simple.json"
...
def _generate_header(diagram_title, **kwargs):
    ...
    header_lines = [
        ...
        "}",
        "",
        f"skinparam TitlefontColor  {config['spatial_table_color']}",
...
def _generate_table_definition(table_name, table_object, config):
    ...
    if hasattr(table_object, "relation") and table_object.relation:
        ...
        lines.append(
            f"entity \"{display_name} \" as {alias_name} <<spatial>> {config['spatial_table_color']} {{"
        )
    else:
        lines.append(
...
def main():
    ...
    all_tables = process_database_tables(
        db_schema, IGNORE_OBJECTS, IGNORE_FIELDS, logger
    )
    ...
    puml_file_path = os.path.join(OUTPUT_DIR, "ER-GCOVER.puml")
    logger.info(f"Writing PUML diagram to {puml_file_path}")
